...
# glm
logit <- glmer(train_data$pVolsOX ~ train_data$Gender + train_data$attn + train_data$unatt + train_data$cont + train_data$Int + train_data$Vols + train_data$Touch + train_data$Excel + train_data$Diff + (1|train_data$ID), family = "binomial", control=glmerControl(optimizer = "bobyqa"), nAGQ=1)
...
write.csv(result,file="glm_result.csv")
